class LiquidToggleBehavior extends LiquidBehavior {
    ...
    changeOpenedItems(e) {
        ...
        let l = _.uniq([...r, e]);
        s && (l = [e]), r.includes(e) && !g && (l = l.filter(n => n !== e)), i && (l = this.getUI("togglableTriggers").map((n, o) => o).filter((n, o) => !r.includes(o))), this.view.model.set({
            [t]: l
        })
    }
    onLqdWidgetChangeOpenedItems(e) {
        if (this.getOption("ignoreEnterOnFocus") && e.pointerType === "" && this.getUI("togglableTriggers").findIndex(g => g === document.activeElement) < 0 || this.getOption("disableOnTouch") && liquidTouchMM.matches) return;
    ...
    onChangeOpenedItems() {
        ...
        this.handleClassnames(), this.view.trigger(`change:${this.getChangeProp()}`, this.openedElements), this.view.trigger(`change:${this.getChangeProp("closedItems")}`, this.closedElements), e && (e.set("toggle:open", {
...
window.liquid ? .app ? .model ? .set("loadedBehaviors", [...window.liquid.app.model.get("loadedBehaviors"), LiquidToggleBehavior]);